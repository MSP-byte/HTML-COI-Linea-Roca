...
mod = once(mod, anchor, insert, 'helpers tipo')

anchor = """  const fechaHora = (v) => {
...
mod = once(mod, anchor, insert, 'helpers fecha')

old = """  function etapasEtapa2() {
    const todas = etapas();
...
new = """  function etapasEtapa2(orden) {
    const todas = etapas();
    const extra = etapaExtraSaldo();
    const lista = todas.filter((e) => CODIGOS_ETAPA2.indexOf(e.codigo) >= 0)
...
mod = once(mod, old, new, 'etapas etapa2 por tipo')
...
mod = once(mod, """    return {
      ...
      nro: nro,
      orden: orden,
      hitos: hitos,""", 'estado conserva orden')
mod = once(mod, '    const lista = etapasEtapa2();', '    const lista = etapasEtapa2(estado.orden);', 'bloque etapa2 lista')
...
mod = mod.replace(old_name, "'<span class=\\"etapa1-cuerpo\\"><span class=\\"etapa1-nombre\\">' + esc(nombreEtapaPorTipo(et,estado.orden)) + '</span>' +", 2)

count_dates = mod.count('fechaHora(ev.fecha_evento)')
if count_dates < 3:
    raise SystemExit(f'fechaHora eventos: se esperaban >=3 y hubo {count_dates}')
...
old = """      if (esActual && !estado.etapa1Finalizada) return diasEntre(x.ev.fecha_evento, null);
...
new = """      if (esActual && !estado.etapa1Finalizada) return diasEntre(fechaCalculoEvento(x.ev), null);
      // Hueco: el inmediato siguiente no esta registrado.
      return null;
    }
...
mod = once(mod, old, new, 'dias efectivos')

mod = once(mod, """    const estado = estadoPipeline(orden || {});
    // Advertencia por salto:""", """    const estado = estadoPipeline(orden || {});
    const eventoExistente = estado.porCodigo.get(codigo) || null;
    const fechaDefault = fechaInputEvento(eventoExistente) || hoyBuenosAires();
    // Advertencia por salto:""", 'fecha modal default')
mod = once(mod, "'<dt>Hito</dt><dd id=\\"etapa1ModalHito\\">' + esc(etapa.nombre) + '</dd>' +",
                 "'<dt>Hito</dt><dd id=\\"etapa1ModalHito\\">' + esc(nombreEtapaPorTipo(etapa,orden)) + '</dd>' +", 'modal nombre')
