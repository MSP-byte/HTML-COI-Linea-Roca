-- COI Linea Roca - corrige la validacion portable de actualizado_en en restore exacto.
-- Forward-only sobre 012: evita escapes dependientes del motor/standard_conforming_strings.
  ...
  if exists (
    select 1
     ...
     where nullif(source.value ->> 'actualizado_en', '') is null
        or (source.value ->> 'actualizado_en') !~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}T'
  ) then
...
comment on function public.coi_timeline_replace_events(jsonb) is
  'Restore Timeline atomico y serializado; preserva actualizado_en y valida timestamps ISO con patron portable.';
