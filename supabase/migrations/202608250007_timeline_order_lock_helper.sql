-- COI Linea Roca - helper acotado para bloquear OC sin reabrir DML directo.
...
comment on function public.coi_timeline_lock_orders(jsonb) is
  'Helper restringido por rol que toma locks de OC en orden estable antes del upsert Timeline.';
...
begin
  ...
  -- El helper solo toma locks y vuelve a validar el rol. La mutacion queda en
  -- esta funcion SECURITY INVOKER, por lo que RLS y privilegios siguen activos.
...
comment on function public.coi_timeline_upsert_events(jsonb) is
  'Upsert invoker con versionado y helper restringido para locks ordenados de OC.';
