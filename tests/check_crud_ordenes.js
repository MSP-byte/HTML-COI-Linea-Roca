...
(async()=>{
  const adapterBody = name => {
    const match=html.match(new RegExp(`async function ${name}\\(\\)\\{([\\s\\S]*?)\\n\\s*\\}`));
    assert.ok(match,`No se encontró el adaptador async ${name}`);
    return match[1];
  };
  const forbiddenLegacy=/estaciones\.forEach|guardarBaseLocal|\.filter\s*\(|localStorage|Borrado correcto/;
  for(const name of ['v45BorrarSeleccionadas','borrarOrdenesSeleccionadasR4']){
    const body=adapterBody(name);
  ...
  m=new MockStore(fixtures,{rls:true}); r=await m.remove([fixtures[0]]); assert.strictEqual(r.status,'rls'); assert.strictEqual(m.orders.length,3); assert.deepStrictEqual(m.cache,fixtures); assert.strictEqual(m.messages.filter(x=>/eliminad[ao] correctamente|Borrado correcto/i.test(x)).length,0);
