...
assert.match(moduleSource, /confirmarEliminacionNativa\(deletable\)[\s\S]*?await requireAccess\(\)/, 'La confirmación nativa debe ocurrir antes de validar Supabase/permisos');
assert.ok(!/crudOcDeleteModal|confirmDeletion|data-crud-confirm|data-crud-cancel/.test(moduleSource), 'No debe quedar ningún camino del modal roto');
assert.match(moduleSource, /window\.confirm\(/);
assert.match(moduleSource, /window\.prompt\('Escriba ELIMINAR para confirmar:'\)/);
assert.match(moduleSource, /window\.prompt\('Ingrese el PIN de Administrador:'\)/);
assert.match(moduleSource, /const number=selectedNumber\(checkbox\)/);
assert.match(moduleSource, /orders\.filter\(order=>nro\(order\)===number\)/);
