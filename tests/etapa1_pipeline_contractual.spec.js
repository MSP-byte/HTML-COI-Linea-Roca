...
test('E1-6 · OC histórica con fecha de acta mantiene la 2° Etapa habilitada',async({page})=>{await setup(page,{fecha_acta_inicio:'2025-06-30'});const e=await estado(page);expect(e.etapa2).toBe('si');expect(e.avance).toBe('0 / 8')});
...
test('E1-11 · I · si Supabase falla, la interfaz no simula éxito',async({page})=>{await setup(page,{fallaRpc:true});await page.click('[data-etapa1-hito="pliegos_preparacion"]');await page.click('#etapa1ModalConfirmarBtn');await expect(page.locator('#etapa1ModalError')).toBeVisible();expect((await estado(page)).avance).toBe('0 / 8')});

// H/O y hardening
test('E1-12 · H · tras recargar, el pipeline se reconstruye desde el historial remoto',async({page})=>{await setup(page,{historial:[EVENTO('pliegos_preparacion','2026-09-01T10:00:00Z'),EVENTO('pliegos_terminado_sin_solped','2026-09-03T10:00:00Z')]});expect((await estado(page)).avance).toBe('2 / 8');await page.reload({waitUntil:'domcontentloaded'});await page.waitForFunction(()=>typeof window.__COI_ETAPA1_RENDER__==='function');await pintar(page);expect((await estado(page)).avance).toBe('2 / 8')});
...
test('E1-18 · F3 · un hueco en la secuencia no produce una duración inventada',async({page})=>{await setup(page,{historial:[EVENTO('solped_sin_expediente','2026-09-01T10:00:00Z'),EVENTO('pliego_con_expediente','2026-09-11T10:00:00Z')]});expect(await page.locator('[data-etapa1-hito="solped_sin_expediente"] .etapa1-dias').textContent()).toContain('—')});
