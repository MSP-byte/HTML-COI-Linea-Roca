...
replacement = r'''  function renderAlertsExecutive(){
    const host=document.querySelector('#vistaCentroAlertas .view-body');
    if(!host)return;

    const mainTable=host.querySelector('table.coi-alertas-table');
    if(!mainTable){
      if(!host.__coiExecAlertsWaiter){
        const observer=new MutationObserver(()=>{
          if(!host.querySelector('table.coi-alertas-table'))return;
          observer.disconnect();
          host.__coiExecAlertsWaiter=null;
          requestAnimationFrame(()=>renderAlertsExecutive());
        });
        host.__coiExecAlertsWaiter=observer;
    ...
    if(host.__coiExecAlertsWaiter){
      host.__coiExecAlertsWaiter.disconnect();
      host.__coiExecAlertsWaiter=null;
    }
